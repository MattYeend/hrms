...
class Logger extends Model
{
    ...
    const ACTION_MEETING_MEETING_TYPE_CHANGE = 124;

    const ACTION_CREATE_MEETING_TYPE = 125;
    const ACTION_UPDATE_MEETING_TYPE = 126;
    const ACTION_DELETE_MEETING_TYPE = 127;
    const ACTION_SHOW_MEETING_TYPE = 128;

    const ACTION_CREATE_LOCATION = 129;
    const ACTION_UPDATE_LOCATION = 130;
    const ACTION_DELETE_LOCATION = 131;
    const ACTION_SHOW_LOCATION = 132;
    
    const ACTION_CHANGE_MEETING_LOCATION = 133;
    const ACTION_CHANGE_MEETING_VIRTUAL_TO_PERSON = 134;
    const ACTION_CHANGE_MEETING_PERSON_TO_VIRTUAL = 135;

    const ACTION_PROFILE_PICTURE_UPLOAD = 136;
    const ACTION_PROFILE_PICTURE_CHANGE = 137;
    const ACTION_CV_UPLOAD = 138;
    const ACTION_CV_CHANGE = 139;
    const ACTION_COVER_LETTER_UPLOAD = 140;
    const ACTION_COVER_LETTER_CHANGE = 141;

    const ACTION_USER_JOB_CHANGE = 142;
    const ACTION_USER_ROLE_CHANGE = 143;
    const ACTION_USER_PROMOTION = 144;
    const ACTION_USER_DEMOTION = 145;
    const ACTION_USER_CHANGE_OF_ADDRESS = 146;
    const ACTION_USER_REMOTE_TO_OFFICE = 147;
    const ACTION_USER_OFFICE_TO_REMOTE = 148;
    const ACTION_USER_REMOTE_TO_HYBRID = 149;
    const ACTION_USER_HYBRID_TO_REMOTE = 150;
    const ACTION_USER_OFFICE_TO_HYBRID = 151;
    const ACTION_USER_HYBRID_TO_OFFICE = 152;

    const ACTION_COMPANY_RELATIONSHIP_MANAGER_ASSIGNED_TO_COMPANY = 153;
    const ACTION_COMPANY_RELATIONSHIP_MANAGER_CHANGE_COMPANY_TO_COMPANY = 154;
    const ACTION_COMPANY_RELATIONSHIP_MANAGER_REMOVE_FROM_COMPANY = 155;

    // Meeting invite sent to user
    const ACTION_MEETING_INVITE_EMAIL_SENT = 156;
    // Meeting is accepted on frontend
    const ACTION_MEETING_RESPONSE_ATTEND = 157;
    // Meeting is maybe on frontend
    const ACTION_MEETING_RESPONSE_MAYBE = 158;
    // Meeting is declined on frontend
    const ACTION_MEETING_RESPONSE_DECLINE = 159;
    // Meeting invite email responses
    const ACTION_MEETING_INVITE_EMAIL_ACCEPT_RESPONSE = 160;
    const ACTION_MEETING_INVITE_EMAIL_MAYBE_RESPONSE = 161;
    const ACTION_MEETING_INVITE_EMAIL_DECLINE_RESPONSE = 162;
    // Meeting response changes
    const ACTION_INVITE_ATTEND_TO_MAYBE = 163;
    const ACTION_INVITE_ATTEND_TO_DECLINE = 164;
    const ACTION_INVITE_MAYBE_TO_ATTEND = 165;
    const ACTION_INVITE_MAYBE_TO_DECLINE = 166;
    const ACTION_INVITE_DECLINE_TO_ATTEND = 167;
    const ACTION_INVITE_DECLINE_TO_MAYBE = 168;
