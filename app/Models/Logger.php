...
class Logger extends Model
{
    ...
    const ACTION_APPROVE_BLOG = 135;
    const ACTION_DENY_BLOG = 136;

    // Blog category
    const ACTION_CREATE_BLOG_CATEGORIES = 137;
    const ACTION_UPDATE_BLOG_CATEGORIES = 138;
    const ACTION_DELETE_BLOG_CATEGORIES = 139;
    const ACTION_SH0W_BLOG_CATEGORIES = 140;

    // Blog interactions
    const ACTION_BLOG_LIKE = 141;
    const ACTION_BLOG_COMMENT = 142;

    // Licences 
    const ACTION_CREATE_LICENCE = 143;
    const ACTION_UPDATE_LICENCE = 144;
    const ACTION_DELETE_LICENCE = 145;
    const ACTION_SHOW_LICENCE = 146;
    const ACTION_COMPANY_LICENCE_CHANGE = 147;

    // Meetings
    const ACTION_CREATE_MEETING = 148;
    const ACTION_UPDATE_MEETING = 149;
    const ACTION_DELETE_MEETING = 150;
    const ACTION_SHOW_MEETING = 151;

    // Meeting types
    const ACTION_MEETING_MEETING_TYPE_CHANGE = 152;
    const ACTION_CREATE_MEETING_TYPE = 153;
    const ACTION_UPDATE_MEETING_TYPE = 154;
    const ACTION_DELETE_MEETING_TYPE = 155;
    const ACTION_SHOW_MEETING_TYPE = 156;

    // Meeting locations
    const ACTION_CHANGE_MEETING_LOCATION = 157;
    const ACTION_CHANGE_MEETING_VIRTUAL_TO_PERSON = 158;
    const ACTION_CHANGE_MEETING_PERSON_TO_VIRTUAL = 159;

    // Locations
    const ACTION_CREATE_LOCATION = 160;
    const ACTION_UPDATE_LOCATION = 161;
    const ACTION_DELETE_LOCATION = 162;
    const ACTION_SHOW_LOCATION = 163;

    // Meeting invite sent to user
    const ACTION_MEETING_INVITE_EMAIL_SENT = 164;
    // Meeting response frontend
    const ACTION_MEETING_RESPONSE_ATTEND = 165;
    const ACTION_MEETING_RESPONSE_MAYBE = 166;
    const ACTION_MEETING_RESPONSE_DECLINE = 167;
    // Meeting invite email responses
    const ACTION_MEETING_INVITE_EMAIL_ACCEPT_RESPONSE = 168;
    const ACTION_MEETING_INVITE_EMAIL_MAYBE_RESPONSE = 169;
    const ACTION_MEETING_INVITE_EMAIL_DECLINE_RESPONSE = 170;
    // Meeting response changes
    const ACTION_INVITE_ATTEND_TO_MAYBE = 171;
    const ACTION_INVITE_ATTEND_TO_DECLINE = 172;
    const ACTION_INVITE_MAYBE_TO_ATTEND = 173;
    const ACTION_INVITE_MAYBE_TO_DECLINE = 174;
    const ACTION_INVITE_DECLINE_TO_ATTEND = 175;
    const ACTION_INVITE_DECLINE_TO_MAYBE = 176;

    // Throttling
    const ACTION_THROTTLING_LOGIN_ATTEMPTS = 177;
    const ACTION_THROTTLING_PASSWORD_RESET = 178;
    const ACTION_THROTTLING_REGISTER = 179;

    // Notifications
    const ACTION_NOTIFICATION_NEW = 180;
    const ACTION_NOTIFICATION_READ = 181;
    const ACTION_NOTIFICATION_IGNORE = 182;

    // Rotas
    const ACTION_CREATE_ROTA = 183;
    const ACTION_UPDATE_ROTA = 184;
    const ACTION_DELETE_ROTA = 185;
    const ACTION_SHOW_ROTA = 186;

    // Address Books
    const ACTION_CREATE_ADDRESS_BOOK = 187;
    const ACTION_UPDATE_ADDRESS_BOOK = 188;
    const ACTION_DELETE_ADDRESS_BOOK = 189;
    const ACTION_SHOW_ADDRESS_BOOK = 190;

    // Address Book Contacts
    const ACTION_CREATE_ADDRESS_BOOK_CONTACT = 191;
    const ACTION_UPDATE_ADDRESS_BOOK_CONTACT = 192;
    const ACTION_DELETE_ADDRESS_BOOK_CONTACT = 193;
    const ACTION_SHOW_ADDRESS_BOOK_CONTACT = 194;

    // Company Contracts
    const ACTION_CREATE_CONTRACT = 195;
    const ACTION_UPDATE_CONTRACT = 196;
    const ACTION_DELETE_CONTRACT = 197;
    const ACTION_SHOW_CONTRACT = 198;

    // Errors
    const ACTION_GENERAL_ERRORS = 199;
